    ...
    try:
        while True:
            if v1 == 2:
                ...
                sock.send(createheader(5,0,0,0))
            elif v1 == 6:
                senddict(sock)
            data = sock.recv(6)
            v1, v2, v3, v4 = struct.unpack('>bbhh', data)
    except ConnectionResetError:
        del clientsdict[[i for i in clientsdict if clientsdict[i]==sock][0]]
    ...
    try:
        while True:
            ...
            if v1 == 0:
                ...
                conn_socket.send(createheader(5,0,0,0))
                if v2 == 1:
                    senddict(conn_socket)
                conn_socket.close()
                break
    except ConnectionResetError:
        serversdict.remove(conn_socket)
    #conn_socket.close()
